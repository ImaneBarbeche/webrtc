...
export const stateToQuestionMap = {
  ...
  askAlwaysLivedInCountry: "Avez-vous toujours vécu dans ce pays ?",
  askSameHousingInCountry:
    "Avez-vous toujours vécu dans le même logement dans ce pays ?",
  askMultiplePlaces:
    "Pouvez-vous citer les lieux/pays/villes où vous avez vécu ?",
  askPlaceArrivalYear:
    "En quelle année êtes-vous arrivé dans ce lieu/pays/ville ?",
  askPlaceDepartureYear:
    "En quelle année avez-vous quitté ce lieu/pays/ville ?",
  askSameHousingInPlace:
    "Avez-vous toujours vécu dans le même logement dans ce lieu/pays/ville ?",
  ...
  askMultipleCommunes:
    "Pouvez-vous citer les communes dans lesquelles vous avez vécu ?",
  ...
  askHousingDepartureAge:
    "À quel âge ou en quelle année avez-vous quitté ce logement ?",
  askHousingOccupationStatusEntry:
    "Quel était votre statut d'occupation à l'arrivée ?",
  askHousingOccupationStatusExit:
    "Quel était votre statut d'occupation au départ ?",
...
export const surveyMachine = createMachine(
    ...
    states: {
      ...
      askAlwaysLivedInCountry: {
        on: {
          ...
          NO: {
            actions: [assign({ alwaysLivedInCountry: () => "No" })],
            target: "askMultiplePlaces",
          },
        },
      },
      askSameHousingInCountry: {
      ...
      askMultiplePlaces: {
        on: {
          ANSWER_MULTIPLE_PLACES: {
            actions: ["addMultiplePlaces"],
            target: "placeNextPlaceOnTimeline",
          },
        },
      },
      placeNextPlaceOnTimeline: {
        always: [
          {
          ...
          {
            actions: ["resetPlaceIndex"],
            target: "askSameHousingInPlace",
          },
        ],
      },
      askSameHousingInPlace: {
      ...
      askPlaceArrivalYear: {
        on: {
          ANSWER_PLACE_ARRIVAL: {
            actions: ["addCalendarEpisode"],
            target: "askPlaceDepartureYear",
          },
        },
      },
      askPlaceDepartureYear: {
        on: {
          ANSWER_PLACE_DEPARTURE: {
            actions: ["modifyCalendarEpisode", "nextPlace"],
            target: "placeNextPlaceOnTimeline",
          },
        },
      },
  ...
  },
  {
    actions: {
      ...
      addCountry: assign(({ context, event }) => {
        return {
          ...context,
      ...
      modifyCalendarEpisode: assign({
        lastEpisode: ({ context, event }, params) => {
          ...
          if (!context.lastEpisode || !context.lastEpisode.id) {
            console.warn(
              "lastEpisode est null, impossible de modifier l'épisode"
            );
            return null;
          }
      ...
      nextPlace: assign({
        currentPlaceIndex: ({ context }) =>
          (context.currentPlaceIndex || 0) + 1,
      }),
      resetPlaceIndex: assign({
        currentPlaceIndex: () => 0,
      }),
      ...
      nextGroup: assign({
        //A Modifier, marche que pour 14,13,12,11
        group: ({ context, event }) => {
