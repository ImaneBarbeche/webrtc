...
function createNormalizedEvent(type, data = {}) {
  ...
  switch (type.toUpperCase()) {
    case "ANSWER_BIRTH_YEAR":
      ...
      if (data.birthdate) {
        const yearMatch = String(data.birthdate)
          .trim()
          .match(/^(\d{4})/);
        event.birthdate = yearMatch
          ? yearMatch[1]
    ...
    case "ANSWER_NEW_COMMUNE":
      ...
      if (data.commune) {
        event.commune = Array.isArray(data.commune)
          ? data.commune
          : [data.commune];
    ...
    case "ANSWER_HOUSING_ARRIVAL_YEAR":
      ...
      if (data.start) {
        const yearMatch = String(data.start)
          .trim()
          .match(/^(\d{4})/);
    ...
    case "ANSWER_HOUSING_DEPARTURE_YEAR":
      ...
      if (data.end) {
        const yearMatch = String(data.end)
          .trim()
          .match(/^(\d{4})/);
    ...
    case "ANSWER_HOUSING_SPLIT_YEAR":
      ...
      if (data.split) {
        const yearMatch = String(data.split)
          .trim()
          .match(/^(\d{4})/);
...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  function renderQuestion(state) {
    ...
    switch (state.value) {
      ...
      case "askNewCommune":
        questionText =
          "Pouvez-vous nous indiquer les communes et départements (ou pays si étranger) de vos différents lieux de résidence puis les placer ?";
        responseType = "inputlist";
        eventType = "ANSWER_NEW_COMMUNE";
        eventKey = "commune";
        break;
      case "askCommuneArrivalAge":
        questionText = `À quel âge êtes-vous arrivé dans ${
          state.context.communes[state.context.currentCommuneIndex]
        } ?`;
        responseType = "input";
        eventType = "ANSWER_COMMUNE_ARRIVAL_AGE";
        eventKey = "age";
        break;
      case "askCommuneArrivalYear":
        ...
        responseType = "input";
        eventType = "ANSWER_COMMUNE_ARRIVAL_YEAR";
        eventKey = "start";
        break;
      case "askArrivalYear":
        questionText = `En quelle année êtes-vous arrivé à ${
        ...
        break;
      case "askSameHousing":
        questionText = `Avez-vous toujours vécu dans le même logement à ${
          state.context.communes[state.context.currentCommuneIndex]
        } ?`;
        responseType = "choice";
        choices = ["Yes", "No"];
      ...
      case "askChangeHousingStatus":
        questionText =
          "Avez-vous changé de statut d'occupation entre l'entrée et la sortie du logement ?";
        responseType = "choice";
        choices = ["Yes", "No"];
        break;
      case "surveyComplete":
    ...
    if (responseType === "input") {
      ...
      input.addEventListener("keypress", (event) => {
        if (event.key === "Enter" && input.value.trim() !== "" && eventType) {
          ...
          const rawData = {};
          rawData[eventKey] =
            eventKey === "commune" ? [input.value] : input.value;
    ...
    // Gestion des réponses avec un input et une liste
      ...
      nextQBtn.addEventListener("click", () => {
        ...
        responseList
          .querySelectorAll("li")
          .forEach((e) => list_communes.push(e.innerHTML));

        console.log(
          "📝 Communes envoyées dans l'ordre de saisie:",
          list_communes
        );
        console.log(
          "⚙️ L'ordre sera synchronisé avec la timeline par la state machine"
        );

        // Normalisation et envoi
        const normalizedEvent = createNormalizedEvent(eventType, {
